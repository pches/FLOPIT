...
# setwd("~/...<where the main repo is saved on your computer>.../FLOPIT-master")
...
allnas <- c(which(is.nan(zones)==TRUE), 
            which(is.nan(probabilities)==TRUE),
            which(is.na(zones)==TRUE), 
            which(is.na(probabilities)==TRUE))
